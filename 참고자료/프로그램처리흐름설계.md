...
```
[InputHandler] → [Tetromino] → [GameBoard]
     ↓                ↓              ↓
  키 입력         블록 이동      충돌 확인
                    ↓
              [GameController]
...
InputHandler.Update()
   - Input.GetKey() 감지
   ↓
Tetromino.MoveLeft() 또는 MoveRight()
   ↓
ValidMove() 호출
   ↓
...
**예시**:
```csharp
// Tetromino에서 GameBoard 찾기
gameBoard = FindObjectOfType<GameBoard>();

// Tetromino에서 GameController 찾기
...
- **실행 순서**: InputHandler → Tetromino → GameController → 렌더링
...
프레임 1:
  ...
  - Tetromino: fallTimer = 0.016초 (아직 fallTime 미도달)
  - 렌더링: 현재 상태 그리기

프레임 2:
  - InputHandler: ← 키 입력 → MoveLeft() 호출
  - Tetromino: fallTimer = 0.032초
  - 렌더링: 이동한 블록 그리기

프레임 60:
  - InputHandler: 키 입력 없음
  - Tetromino: fallTimer = 0.96초 (아직 fallTime 미도달)
  - 렌더링: 현재 상태 그리기

프레임 61:
  - InputHandler: 키 입력 없음
  - Tetromino: fallTimer = 0.976초 → fallTime(1초) 도달 → MoveDown() 호출
  - 렌더링: 아래로 이동한 블록 그리기
```
