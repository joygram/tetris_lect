...
{
    public GameObject[] tetrisBlockPrefabs;  // 7가지 테트리스 블록 프리팹 배열
...
**Unity 에디터에서 설정**:
1. 빈 GameObject 생성 (이름: "Spawner")
2. `Spawner` 스크립트 추가
3. Inspector에서:
   - `TetrisBlock Prefabs` 배열 크기를 7로 설정
   - 각 슬롯에 7가지 테트리스 블록 프리팹 할당
   - `Spawn Point`는 빈 Transform 오브젝트 생성 후 할당 (또는 null로 두고 코드에서 계산)
