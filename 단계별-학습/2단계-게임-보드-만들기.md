...
#### 3-3. Transform 위치 설정 — 논리 좌표와 맞추기

1. **[Hierarchy > GameBoard]** 클릭

2. **[Inspector > GameBoard > Transform]** 에서 **Position** 확인
   - **X: 0, Y: 0, Z: 0** 으로 두세요.
   - 이렇게 해야 **논리 그리드 (0,0)~(9,19)** 와 **씬(월드) 좌표**가 일치합니다. (블록·보드 배경이 나중에 같은 좌표계를 쓰게 됩니다.)

3. **값이 다르다면**
   - Position 필드에 **0, 0, 0** 을 입력해 (0, 0, 0)으로 맞춥니다.
   - GameBoard를 다른 위치에 두고 싶다면 7단계 [보드 논리좌표와 시각 일치](./7단계-점수-레벨-시스템.md#board-logical-coords)에서 좌표 헬퍼를 쓰면 됩니다.

**💡 요약**: GameBoard Position **(0, 0, 0)** = 논리 좌표와 씬 좌표를 같게 둔 상태. 이걸로 두면 6단계에서 보드 시각화(배경)를 붙일 때도 좌표가 정확히 맞습니다.
