...
**카메라가 이상하면** (안 보임·너무 멀다·비스듬함·검은 화면) → [카메라 설정 가이드 - 카메라가 이상할 때](../참고자료/카메라-설정-가이드.md#카메라가-이상할-때-먼저-확인) 참고.
...
1. **Game 뷰 포커스**
   **Game** 뷰를 한 번 **클릭**해서 포커스를 주세요. 에디터 다른 창에 포커스가 가 있으면 키 입력이 게임에 전달되지 않아서, “처음만 되고 그 다음엔 안 된다”처럼 느껴질 수 있습니다.
2. **블록이 씬에 한 개만 있는지**
   **Hierarchy**에 TetrisBlock이 붙은 블록이 **두 개** 있으면 (예: 수동으로 넣은 블록 + Spawner가 만든 블록), `FindObjectOfType<TetrisBlock>()`이 둘 중 하나만 찾습니다. 회전·이동이 적용되는 블록과 보이는 블록이 달라질 수 있습니다. **Spawner를 쓰는 경우** Hierarchy에 **수동으로 끌어다 둔 블록은 제거**하고, 스폰된 블록 하나만 두세요.
3. **회전 후 보드 밖으로 나갔는지**
   회전을 여러 번 하면 블록이 **보드 밖**(그리드 범위 밖)으로 나갈 수 있습니다. 그러면 `ValidMove()` / 회전 가능 여부가 false가 되어 **이동·추가 회전이 모두 막힙니다**.  
   → **Spawn Point** 위치를 보드 **중앙 위쪽**으로 두세요. (예: Position **(5, 18, 0)**)  
   → **GameBoard**의 **Width / Height**가 10, 20인지 확인.  
   → 이미 밖으로 나간 상태라면 **Play를 멈추고** Spawn Point 위치를 조정한 뒤 다시 Play.
4. **Console에 에러가 있는지**
   **NullReferenceException** 등이 있으면 **GameBoard**를 못 찾은 경우일 수 있습니다. [Hierarchy > GameBoard]에 **Game Board (Script)** 가 붙어 있는지, 작업 씬이 **Tetris**인지 확인하세요.

**정리**: **Game 뷰 클릭(포커스)** → **블록 한 개만** (수동 블록 제거) → **Spawn Point·보드 크기**로 회전 후에도 보드 안에 있게 하기 → **Console 에러** 확인.
