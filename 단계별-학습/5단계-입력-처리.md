...
**체크 방법**  
- **GameBoard**: Hierarchy에서 "GameBoard" 클릭 → Inspector에 "Game Board (Script)" 있는지  
- **Spawner**: Hierarchy에서 "Spawner" 클릭 → Inspector에 "Spawner (Script)", "Spawn Point", "Block Prefab" 채워져 있는지  
- **블록 프리팹**: Project에서 I_Block(등) 더블클릭 → 프리팹 루트(맨 위 오브젝트) 선택 → Inspector에 "Tetris Block (Script)" 있는지  
...
1. **Project** 창에서 **Scripts** 폴더 선택 → **우클릭** → **Create** → **C# Script** → 이름 **InputHandler**
2. **InputHandler.cs** 더블클릭해서 열기 → 아래 코드로 **전체 교체** 후 저장 (Ctrl+S)
...
1. **빈 오브젝트 만들기**  
   **Hierarchy** 창의 **빈 곳**(다른 오브젝트 이름이 아닌, 빈 공간)에서 **우클릭** → **Create Empty**를 클릭한다.  
   → 새 오브젝트가 **루트 레벨**(Main Camera, GameBoard와 같은 줄)에 **GameObject**로 생긴다.

2. **이름 바꾸기**  
   방금 생긴 오브젝트를 **한 번 클릭**해서 선택한 뒤, **F2**를 누르거나 이름 칸을 한 번 더 클릭해서 **InputHandler**라고 입력하고 **Enter**로 확정한다.  
   (Inspector 창 맨 위 **이름** 칸에서 바꿔도 된다.)

3. **InputHandler 스크립트 붙이기 (드래그)**  
   **Project** 창에서 **Scripts** 폴더를 연 다음 **InputHandler** 스크립트를 **클릭한 채로** 잡는다.  
   그 상태로 **Hierarchy** 창으로 끌어가서 **InputHandler** 오브젝트 **위에** 올린 뒤 **손을 뗀다** (드롭).  
   → Inspector에 **Input Handler (Script)** 가 붙으면 연결된 것이다.

4. **확인**  
   **Hierarchy**에서 **InputHandler**를 선택했을 때 **Inspector**에 **Input Handler (Script)** 가 보이면 씬에 제대로 연결된 것이다.  
   InputHandler는 Spawner와 달리 Inspector에서 **연결할 칸**(Spawn Point, Block Prefab 같은 것)이 없다. 스크립트만 붙이면 된다.

5. **씬 저장**  
   **Ctrl+S**를 누르거나 **File** → **Save**를 선택해서 씬을 저장한다.  
   저장하지 않으면 나중에 씬을 다시 열었을 때 InputHandler 오브젝트가 없을 수 있다.

**정리**: Hierarchy **루트 레벨**에 **InputHandler** 빈 오브젝트를 만든 뒤 → **Project**의 **InputHandler.cs**를 **Hierarchy**의 **InputHandler** 오브젝트 위로 **드래그**해서 스크립트를 붙이고 → **Ctrl+S**로 씬을 저장하면 씬에 InputHandler가 연결된 것이다.
...
1. **▶ Play를 눌러야 합니다.**  
   에디터에서 **Play**를 누르면 게임이 실행되고, 그때부터 Spawner가 블록을 하나 스폰하고, 아래 동작이 돌아갑니다.

2. **자동 낙하 (이미 4단계에서)**  
   스폰된 블록에 붙어 있는 **TetrisBlock**의 **Update()**가 **매 프레임** 실행됩니다.  
   그 안에서 `fallTimer`가 쌓이고, **fallSpeed**(기본 1초)마다 **MoveDown()**이 호출됩니다.  
   그래서 **키를 안 눌러도** 스폰된 블록이 **저절로 아래로 한 칸씩** 내려갑니다.
...
1. **Game 뷰에 포커스가 있는지**  
   **Game** 뷰 탭을 **한 번 클릭**해서 포커스를 줍니다. Inspector나 다른 창에 포커스가 있으면 키 입력이 게임에 전달되지 않습니다.

2. **InputHandler가 씬에 있는지**  
   **Hierarchy**에 **InputHandler** 오브젝트가 있고, Inspector에 **Input Handler (Script)** 가 붙어 있는지 확인합니다. 없으면 입력을 받는 쪽이 없습니다.

3. **스폰된 블록(프리팹) 루트에 TetrisBlock이 붙어 있는지**  
   Spawner가 스폰하는 건 **블록 프리팹의 루트** 오브젝트입니다. `FindObjectOfType<TetrisBlock>()`은 씬에 있는 **TetrisBlock 컴포넌트** 하나를 찾습니다.  
   → **프리팹을 열어서**, 루트 오브젝트(예: I_Block)에 **TetrisBlock 스크립트**가 붙어 있어야 합니다. 자식 큐브에만 붙어 있고 루트에 없으면 찾지 못해서 이동이 호출되지 않습니다.

4. **GameBoard가 씬에 있는지**  
   **TetrisBlock**은 **Start()**에서 `FindObjectOfType<GameBoard>()`로 GameBoard를 찾습니다. **Hierarchy**에 **GameBoard** 오브젝트가 있고 **Game Board (Script)** 가 붙어 있어야 합니다.  
   GameBoard가 없으면 **gameBoard**가 null이고, **ValidMove()** 안에서 **gameBoard.width** 접근 시 **NullReferenceException**이 나서 이동이 실패합니다. **Console**에 빨간 에러가 뜨는지 확인하세요.
