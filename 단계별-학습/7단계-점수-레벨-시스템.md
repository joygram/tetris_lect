...
**📌 텍스트 드래그가 안 될 때** — UI를 **Text - TextMeshPro**로 만들었다면 Inspector 필드 타입이 **Text(레거시)** 와 달라서 드래그가 안 됩니다. 아래 코드는 **TMP_Text**(TextMeshPro) 타입이라 TMP 오브젝트를 그대로 드래그할 수 있습니다. Legacy **Text**를 쓰면 `TMP_Text` 대신 `UnityEngine.UI.Text`로 바꾸고 `using UnityEngine.UI;` 추가하면 됩니다.
...
public class GameController : MonoBehaviour   // [MonoBehaviour 상속]
{
    ...
    public void AddScore(int linesCleared)   // [우리 메소드] 7단계 4번에서 LockPiece가 호출
    {
        int points = 0;
        switch (linesCleared)
        {
...
**요약**: `gameBoard.ClearFullRows()` → `int linesCleared` 로 받기, 그다음 `FindObjectOfType<GameController>()` → `gameController.AddScore(linesCleared);` 추가. 순서는 GameController 생성(위 2~3번) **다음**에 이 4번을 하면 됩니다.
