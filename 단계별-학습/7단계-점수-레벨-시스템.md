...
| **GameBoard / Spawner / TetrisBlock** | LockPiece에서 ClearFullRows, SpawnNext, AddScore 호출이 연결되어 있나요?                                |

위가 대략 되었다면 7단계는 **GameController + UI Text 연결**만 하면 됩니다.
...
2. **Text 생성**  
   ...
   - **"UI"** → **"Text - TextMeshPro"**(읽는 법: 텍스트메시프로) (또는 **Legacy**(읽는 법: 레거시) **> Text**)  
   - 생성 후: **[Hierarchy > Canvas > Text (TMP)]** 또는 **Text**

3. **TextMeshPro Import (처음 사용 시)**  
   - Import 창이 뜨면 **"Import TMP Essentials"** 클릭 후 완료될 때까지 대기

4. **이름 변경**  
   - **[Hierarchy > Canvas > Text (TMP)]** (또는 Text) 선택  
   - **F2** 또는 **[Inspector]** 맨 위 **이름** 칸에서 **"ScoreText"** 로 변경  
   - 결과: **[Hierarchy > Canvas > ScoreText]**

5. **텍스트 내용 설정**  
   - **[Hierarchy > Canvas > ScoreText]** 클릭  
   - **[Inspector > ScoreText > Text (TextMeshPro)** 또는 **Text (Legacy)**]:
     - **Text**: **"Score: 0"**
     - **Font Size**: **24**
     - **Color**: **흰색**
...
7. **Game 뷰에서 확인**  
   - **상단 Game 탭** 클릭 → 화면 오른쪽 상단에 "Score: 0" 보이면 성공! (여기까지 하면 잠깐 쉬어가도 됩니다.)
...
2. **LinesText 생성**  
   ...
   - 위치: **[Hierarchy > Canvas > Text (TMP)]** (또는 Text)
...
4. **설정**  
   ...
   - **[Inspector > LinesText > Text (TMP)** 또는 **Text**]: **Text** = "Lines: 0", **Font Size** = 24, **Color** = 흰색  
   - **[Inspector > Rect Transform]**: **Pos X** 400, **Pos Y** 150, **Width** 200, **Height** 50

5. **Game 뷰에서 확인**  
   - Game 뷰에서 ScoreText 아래에 "Lines: 0" 보이면 성공!
...
4. **설정**
   ...
   - **[Inspector > LevelText > Text (TMP)** 또는 **Text**]: **Text** = "Level: 1", **Font Size** = 24, **Color** = 흰색
...
- **Unity 프로퍼티**: `scoreText.text`, `linesText.text` — **.text** 는 Text 컴포넌트가 가진 **프로퍼티**로, 화면에 보이는 문자열을 읽거나 씁니다. 괄호 없이 쓰는 값이라 "프로퍼티"입니다.
...
public class GameController : MonoBehaviour   // [MonoBehaviour 상속]
{
    ...
    public Text scoreText;   // [우리 멤버] Inspector에서 연결 — Text는 Unity 타입
    ...
    public void AddScore(int linesCleared)   // [우리 메소드] 6단계 LockPiece에서 호출
    {
        int points = 0;
        switch (linesCleared)
        {
    ...
    {
        if (scoreText) scoreText.text = $"점수: {score}";   // scoreText: [우리 멤버], .text: [Unity 프로퍼티] 상속
...
### 4. TetrisBlock의 LockPiece에서 연결 (코드 연결 확인)

**📌 코드 연결**  
6단계에서 만든 `LockPiece()` 함수 **안에** 이미 아래 코드가 들어 있어야 합니다. 6단계를 그대로 따라했다면 **추가 수정 없이** 7단계에서 **GameController 오브젝트만 씬에 두고** UI Text를 Inspector에서 연결하면 점수가 전달됩니다.

```csharp
// TetrisBlock.cs의 LockPiece() 함수 안에 이미 있어야 함 (6단계에서 추가)
...
**확인**: TetrisBlock.cs의 LockPiece()를 열어 위 세 줄(또는 비슷한 코드)이 **GameBoard.AddToGrid, ClearFullRows 다음**에 있는지 봅니다. 없으면 6단계 [LockPiece 코드](./6단계-행-제거-시스템.md)를 참고해 넣습니다.
