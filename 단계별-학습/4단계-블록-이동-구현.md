...
{
    public GameObject blockPrefab;       // Inspector에서 3단계 프리팹 연결 (예: I_Block)
    ...
    // 4단계: 블록 한 개만 생성. 8단계에서 반복·랜덤·게임오버 추가
...
3. **빈 GameObject 생성** (이름: "Spawner") → 위 스크립트 추가  
4. **Spawn Point**: Hierarchy에서 빈 오브젝트 생성 (이름: SpawnPoint), **Position**을 아래 기준으로 설정하고 Inspector에서 Spawner의 **Spawn Point** 칸에 연결  
   - **권장 좌표**: **(5, 18, 0)** — 2단계 GameBoard가 **width=10, height=20**일 때 **가로 중앙(5)**, **세로 상단 근처(18)** 에 해당합니다.  
   - Unity 월드 좌표 = 보드 논리 좌표로 쓰는 방식이면: x 0~9, y 0~19 안에 들어와야 하므로 (5, 18)은 유효한 스폰 위치입니다.  
5. **Block Prefab**: Inspector에서 **Prefabs 폴더의 블록 프리팹**(예: I_Block)을 **Block Prefab** 칸에 연결  
6. Play → Spawner가 블록 한 개를 생성하고, 그 블록으로 이동·회전 테스트

**📌 생성 위치 확인** (이상하게 보일 때 점검)  
- **블록이 안 보이거나 잘못된 곳에 생성되면**:  
  1. **Spawn Point** 오브젝트를 선택한 뒤 Inspector에서 **Transform > Position**이 **(5, 18, 0)** 인지 확인하세요.  
  2. **GameBoard**의 **width=10, height=20**인지 확인하세요. (2단계에서 만든 값)  
  3. **카메라**가 보드 영역을 비추고 있는지 확인하세요. → 5단계 **[테스트 전 필수 설정](./5단계-입력-처리.md#테스트-전-필수-설정-카메라게임-보드-표현)** 참고.  
