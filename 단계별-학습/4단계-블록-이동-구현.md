...
- **4단계만** 끝냈을 때: **키보드로 좌우·회전**은 아직 없습니다. 4단계에서는 **자동 낙하**(블록이 아래로 떨어지는 것)만 구현합니다.  
  → **키로 움직이게 하려면** [5단계 - 입력 처리](./5단계-입력-처리.md)에서 **InputHandler**를 추가해야 합니다.
- **자동 낙하**가 안 되면 → 아래 2~5를 확인.

### 2. GameBoard가 씬에 있는지

- **TetrisBlock**은 **Start()**에서 `FindObjectOfType<GameBoard>()`로 GameBoard를 찾습니다.
- **[Hierarchy]**에 **GameBoard** 오브젝트가 있고, **[Inspector]**에 **Game Board (Script)**가 붙어 있어야 합니다.
- GameBoard가 없거나 스크립트가 없으면 **gameBoard**가 null이 되어, **ValidMove()** / **Rotate()** 안에서 **gameBoard.width** 접근 시 **NullReferenceException**이 납니다.  
  → **Window → General → Console**에 빨간 에러가 있는지 확인하세요.

### 3. GameBoard Width / Height

- **[Hierarchy > GameBoard]** 클릭 → **[Inspector > Game Board (Script) > Width]** = **10**, **Height** = **20** 인지 확인.  
  기본값이 0이면 이동·낙하 판단이 잘못될 수 있습니다.

### 4. 블록이 씬에 한 개 있는지 (Spawner 사용 시)

- **Spawner**를 쓰는 경우: **[Inspector > Spawner (Script)]**에서 **Block Prefab**, **Spawn Point**가 **둘 다 연결**되어 있어야 합니다. 비어 있으면 블록이 생성되지 않습니다.
- 넣는 **프리팹의 루트 오브젝트**(예: I_Block)에 **TetrisBlock 스크립트**가 붙어 있어야 합니다. 3단계 [TetrisBlock 스크립트를 프리팹에 추가](./3단계-테트리스-블록-만들기.md) 참고.
- **수동**으로 블록을 넣었다면: 그 블록의 **루트**에 **TetrisBlock** 스크립트가 붙어 있고, **같은 씬**에 **GameBoard**가 있어야 합니다.

### 5. 작업 씬이 Tetris인지

- **상단 씬 탭**에서 열린 씬 이름이 **Tetris**인지 확인. 다른 씬이면 GameBoard·블록이 서로 못 찾을 수 있습니다. → [씬 이름 변경 방법](./1단계-Unity-기초.md#scene-rename-guide)

### 6. 블록이 안 보일 때 / 카메라가 이상할 때

- 블록은 생성됐는데 **화면에 안 보이면**: 대부분 **카메라 위치/Size** 문제입니다.  
  → 5단계 **[테스트 전 필수 설정 (카메라·게임 보드 표현)](./5단계-입력-처리.md#테스트-전-필수-설정-카메라게임-보드-표현)** 을 적용하거나, [카메라 설정 가이드 - 카메라가 이상할 때](../참고자료/카메라-설정-가이드.md#카메라가-이상할-때-먼저-확인) 에서 **Position (5, 10, -10)**, **Orthographic**, **Size 10~12** 한 번에 맞추기 참고.
- **Spawn Point** Position을 **(5, 18, 0)** 으로 두면 보드 위쪽에 생성됩니다.

### 7. 5단계까지 했는데 키로 안 움직일 때

- **Game 뷰 포커스**, **InputHandler 씬에 있는지**, **블록 프리팹 루트에 TetrisBlock**, **회전 2번 후 안 됨** 등 → [5단계 - 키를 눌러도 이동이 안 될 때](./5단계-입력-처리.md#키를-눌러도-이동이-안-될-때-떨어지는-건-나중에-하고-좌우회전부터-확인) · [회전 2번 후 아무것도 안 될 때](./5단계-입력-처리.md#회전은-2번-되는데-그-다음엔-아무것도-안-될-때) 참고.

**정리**: **4단계**에서는 자동 낙하만 확인. **GameBoard 씬에 있기** · **Width/Height 10, 20** · **블록 루트에 TetrisBlock** · **Spawner 연결** · **작업 씬 Tetris** · **Console 에러** 순으로 보면 됩니다.
